...
/** Interactive job board. The server page fetches the first page of jobs
 * and passes it as initialJobs so the list is present in crawler-visible
 * HTML; this component takes over with live search/filtering after
 * hydration. */
export default function JobsClient({ initialJobs = [] }) {
  ...
  const { jobs: fetchedJobs, isLoading, error } = useJobs({
    pageSize: 100,
    search: keyword,
    location,
    jobType,
  ...
  // Until SWR resolves, fall back to the server-provided list so the page
  // never flashes a spinner over content the server already rendered.
  const jobs = isLoading && fetchedJobs.length === 0 ? initialJobs : fetchedJobs;
  const showSpinner = isLoading && jobs.length === 0;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-4 sm:px-6 py-6" style={{ backgroundColor: '#faf8f4' }}>
        <div className="max-w-5xl mx-auto">
          ...
          {!showSpinner && !error && visibleJobs.length === 0 && (
